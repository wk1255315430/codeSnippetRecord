...
import {NavBar,BannerConfigNew,TargetTemperatureNew,DeviceControlNew,CustomNew} from '../../components';
import { Toast } from 'antd-mobile';
import { oldAgreementMachineControl } from '../../util/updeviceControl';
...
class HomeNew extends React.Component {
  ...
  newBlowingCalculate(commands){
    ...
    //切换模式到送风时，将风速设置为低风,,强力，静音，静眠关闭
      ...
      // if(window.typeid !== '111c120024000810020200118000010000000000000000000000000000000000'){
      //   commands.push({
      //     'name': 'pmvStatus',
      //     'value': 'true'
      //   });
  ...
  beforeCalculate(commands) {
    ...
    return new Promise((resolve) => {
      ...
      if (operationModeObj.value && operationModeObj.value !== '0' && operationModeObj.value !== '4' && commands[0] && commands[0].name === 'electricHeatingStatus' && commands[0].value === 'true') {
        ...
        Toast.info('该模式下不支持该功能', 1);
      } else if (window.typeid === '00000000000000008080000000041410' || window.typeid === '0000000000000000C040000000041410') {
        // 老typeid空调需要下发单命令
        // let calculateCommands = self.oldModeCalculate(commands);
        self.props.DeviceState.cordovaUtil.updeviceModule.getDeviceInfoById({deviceId:self.props.DeviceState.mac}).then((e)=>{
          if(e.retCode === '000000' && e.retData){
            console.log('e.retData',e.retData.attributes);
            let calculateCommands = oldAgreementMachineControl(commands,e.retData.attributes,self.props.DeviceState.attributes);
            calculateCommands.oldControl = true;
  ...
  calculate(name,value) {
    ...
    this.beforeCalculate([{
      ...
      if(commands.oldControl){//1.0协议底板下发
        console.log('1.0协议底板下发');
        if(commands.flag){
          self.closeSleepingCurve();
        }
        cordovaUtil.updeviceModule.executeOperation({
          deviceId:mac,
          groupName:'004D5F',
          command:commands.commands
        }).then((e)=>{
          console.log('1.0协议下发',e);
        });

      }else{
        cordovaUtil.logicEngineModule.calculateAllPromise(mac, commands).then((res) => {
          cordovaUtil.logicEngineModule.operatePromise(mac);
        });
      }
    });
  ...
  render() {
    ...
    const attributes = this.props.DeviceState.getAttributes();
    console.log('wind',attributes.filter((attr) => attr.name === 'windSpeed')[0]);
    ...
    // }
    // let alermsCancel =localStorage.getItem('hidealarm');
    // if(alermsCancel === 'true'){
