...
let isready = false;
@observer
export default class FunEditorItem extends React.Component {
  constructor(props) {
    super(props);
    this.state={
      items: [],
          ...
          <div style={{display:this.state.editState?'block':'none'}}>
            <List
              ...
              renderItem={({ value, props }) => {
                const row = (
                  <div {...props} className="editorItem">
                    <div style={{backgroundColor:getBasicColor,boxShadow: `0px 3px 4px ${getBasicShadowColor}`}}>
                      <img src={value.iconoff}/>
                    </div>
          ...
          <div style={{display:this.state.editState?'none':'block'}}>
            {
              this.state.items.map( function(item, index) {
                return (
                  <div className="editorItem" key={index}>
                    <div style={{backgroundColor:getBasicColor,boxShadow: `0px 3px 4px ${getBasicShadowColor}`}}>
                      <img src={item.iconoff}/>
                    </div>
                    <span>{item.text}</span>
                  </div>
                );
