...
let seniorFunctionConfig = [{
  ...
  dev: '智慧节能',
  isShow:true,
  image:require('../style/image/custmfunction_energysaving.png')
}, {
  attrName: 'timing',
  dev: '定时开关机',
  isShow:true,
  image:require('../style/image/customfunction_timing.png')
}, {
  attrName: 'energyRecord',
  dev: '能耗记录',
  image:require('../style/image/customfunction_energyconsumptionrecords.png')
}, {
  attrName: 'sleepingCurve',
  dev: '睡眠曲线',
  isShow:false,
  image:require('../style/image/customfunction_sleep.png')
}, {
  attrName: 'faultDetection',
  dev: '故障检测',
  isShow:true,
  image:require('../style/image/customfunction_faultdetection.png')
}, {
  attrName: 'filterCleaning',
  dev: '滤网清洗',
  isShow:false,
  image:require('../style/image/customfunction_filter.png')
}, {
  attrName: 'airConditioningCleaning',
  dev: '空调清洁',
  isShow:false,
  image:require('../style/image/customfunction_acclean.png')
}, {
  attrName: 'purifyingFilter',
  dev: '净化滤芯',
  isShow:false,
  image:require('../style/image/customfunction_jhlx.png')
}];
export default class Custom extends React.Component {
  ...
  async gotoSubpage(path) {
    ...
    if (type === '1' && path === 'energyRecord') {
      ...
      self.offLineToast(path);
    } else if (!isPowerOn) {
      self.powerOffToast(path);
  ...
  render() {
    ...
    // console.log(this.state.hotSeniorArr,'11111111111111111');
    const { prefixCls, attributes,isOnline,cordovaUtil,history} = this.props;
    return (
        ...
        <div className={classnames(`${prefixCls}-main`)}>
          {
            this.state.hotSeniorArr.map( function(item, index) {
              return (
                <div key={index} className={classnames(`${prefixCls}-main-item`)} onClick={()=>{self.gotoSubpage(item.attrName);}}>
          ...
          <div className={classnames(`${prefixCls}-main-item`)} onClick={() => {
            ...
            <div className={classnames(`${prefixCls}-main-item-container`)}>
              <img src={require('../style/image/icon_customfunction_more.png')}/>
              <div>更多</div>
            </div>
          </div>
